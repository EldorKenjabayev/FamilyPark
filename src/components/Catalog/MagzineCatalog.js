...
export default function MagazinePage() {
  ...
  const currentEntertainment = shop.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <Box>
      ...
      >
        ...
        <h3 id="Catalogtitle2">{t("MagazinePage.MagazinePageTitle")}</h3>
      </Box>
      ...
      >
        <h3> {`${currentPage} - ${Math.ceil(shop.length / itemsPerPage)}`}</h3>
        <Box
          sx={{
            display: "flex",
          ...
          >
            <Box
              sx={{
                ...
                marginLeft: `calc(${
                  ((currentPage - 1) /
                    Math.ceil(shop.length / itemsPerPage)) *
                  100
                }%)`,
                width: `${
                  100 / Math.ceil(shop.length / itemsPerPage)
                }%`,
                height: "10px",
                background: "#25254C",
                borderRadius:'10px'
